...
export class NeonStore {
  ...
  async updateLead(id, changes) {
    ...
    if (!rows.length) return null;
    const allowed = ["leadType", "status", "priority", "assignedTo", "feedback", "comments", "lastContactedAt", "nextFollowUpAt"];
    const data = { ...rows[0].data };
    for (const key of allowed) if (changes[key] !== undefined) data[key] = String(changes[key]).slice(0, key === "feedback" || key === "comments" ? 5_000 : 150);
